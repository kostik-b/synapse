...
#include <click/impulse_seq_number.hh>
...
class Packet { public:
    ...
    inline PacketAppType  get_packet_app_type () const;
    inline void           set_packet_app_type (const PacketAppType packet_app_type);

    uint64_t              get_impulse_number  () const;
    //@}
...
//    PacketAppType _packet_app_type;
//    uint64_t      _impulse_seq_number;
    /* mimic Linux sk_buff */
...
//    _packet_app_type    = Synapse::NO_APP_MSG;
//    _impulse_seq_number = Synapse::ImpulseNumber::increment_and_get_next_number ();
}
#endif
...
inline PacketAppType
Packet::get_packet_app_type () const
{
  // packet app type lives in cb at offset 13
  return static_cast<PacketAppType>(anno_s32 (13));
}

inline uint64_t
Packet::get_impulse_number () const
{
...
inline void
Packet::set_packet_app_type (const PacketAppType packet_app_type)
{
  // packet app type lives in cb at offset 13

  //  we don't know the size of packet_app_type
  //  so we simply cast it to 32-bit int

  set_anno_s32 (13, static_cast<int32_t>(packet_app_type));
}
