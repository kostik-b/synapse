...
template<>
  struct hash<SymbolArrayWrapper>
  {
    size_t
    operator()(const SymbolArrayWrapper& key) const
    {
      return *((int32_t*)key._array);
    }
  };
} // namespace __gnu_cxx
...
template<>
  struct equal_to<SymbolArrayWrapper>
  {
    bool operator ()(const SymbolArrayWrapper& lhs, const SymbolArrayWrapper& rhs)
    {
      return strcmp(lhs._array, rhs._array) == 0;
    }
  }; 
